...
class CaptureThread(QThread):
    ...
    def __init__(self, frame_buffer, parent=None):
        ...
        self.update_interval = 0.5 # Update every 0.5 seconds, adjust as needed

    def run(self):
        picam2 = Picamera2()
        # Use a smaller resolution for faster capture and transfer
        ...
        while self.running:
            ...
            # Encode the frame as JPEG with lower quality
            ret, encoded_frame = cv2.imencode('.jpg', frame_rgb, [int(cv2.IMWRITE_JPEG_QUALITY), 50]) # Reduced quality to 50
            if not ret:
                continue
            ...
            try:
                ...
                response = requests.post(
                    ...
                    data=data_to_send,  # Send additional data
                    timeout=2
                )
    ...
    def speak_ar(self):
        try:
            ...
            speak(text_to_speak)  # Use edge_tts_util on the Pi
        except requests.exceptions.RequestException as e:
    ...
    def update_gesture_control(self, gesture, enabled):
        try:
            response = requests.post(f"{self.server_address}/update_gesture", json={"gesture": gesture, "enabled": enabled}, timeout=2)
            response.raise_for_status()
...
class DisplayThread(QThread):
    ...
    def run(self):
        while self.running:
            ...
            time.sleep(0.05)  # Update at approximately 20 FPS
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        try:
            secondary_pixmap_logo = QPixmap("comp_logo.jpg")  # Replace with actual path
